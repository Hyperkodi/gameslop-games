/* Skin loader: turns skin/<name>/skin.json into CSS variables, fonts, strings and the Greek-key frame.
   Generalized from Armaratris' loader: baseDir is now a parameter and the skin.js fallback registry
   is GameSlopKit.skins, but existing skin.js twins that assign window.Armaratris.skins[name] (as
   Armaratris' own skin/armara/skin.js does) are still accepted. */
(function (global) {
  "use strict";
  ...
  function registeredSkin(name) {
    if (K.skins[name]) return K.skins[name];
    if (global.Armaratris && global.Armaratris.skins && global.Armaratris.skins[name]) return global.Armaratris.skins[name];
    return null;
  }

  async function fetchSkin(name, baseDir) {
    const base = baseDir + name + "/";
  ...
  function applySkin(skin) {
    ...
    if (wordmark) wordmark.textContent = skin.wordmark || skin.title.replace(/TRIS$/i, "");
  ...
  async function loadSkin(name, baseDir) {
    baseDir = baseDir || "skin/";
    name = (name || FALLBACK).replace(/[^a-z0-9_-]/gi, "").toLowerCase() || FALLBACK;
    let skin = await fetchSkin(name, baseDir);
    if (!skin && name !== FALLBACK) {
      console.warn("GameSlopKit: skin '" + name + "' not found, falling back to '" + FALLBACK + "'");
      name = FALLBACK;
      skin = await fetchSkin(name, baseDir);
    }
    if (!skin) throw new Error("GameSlopKit: no skin could be loaded");
    skin = JSON.parse(JSON.stringify(skin));
    skin.base = baseDir + name + "/";
    applySkin(skin);
    await preloadLogo(skin);
    return skin;
  }

  Object.assign(K, { loadSkin: loadSkin, frameDataUri: frameDataUri });
