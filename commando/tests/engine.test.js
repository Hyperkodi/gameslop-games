...
test('start config creates independent co-op players and assist lives',()=>{const e=game({players:2,difficulty:'assist'});assert.equal(e.state.players.length,2);assert.equal(e.state.players[0].lives,9);assert.notEqual(e.state.players[0].held,e.state.players[1].held);});
...
test('crouching shrinks the hitbox and stops running',()=>{const e=game();quiet(e);ticks(e,5);const p=e.state.players[0];p.invincible=0;e.input(0,'down',true);e.input(0,'right',true);ticks(e,1);assert.equal(p.prone,true);assert.equal(p.x,110);e.state.bullets.push(enemyBullet(p));ticks(e,1);assert.equal(p.lives,7);});
...
test('barrier prevents bullet deaths but falling still costs a life',()=>{const e=game();quiet(e);const p=e.state.players[0];p.shield=12;p.invincible=0;e.state.bullets.push(enemyBullet(p));ticks(e,1);assert.equal(p.lives,7);p.y=700;ticks(e,1);assert.equal(p.lives,6);});
test('death resets weapon and grants safe checkpoint respawn',()=>{const e=game();quiet(e);const p=e.state.players[0];p.hp=1;p.invincible=0;p.weapon='S';p.rapid=10;e.state.bullets.push(enemyBullet(p));ticks(e,1);assert.equal(p.lives,6);assert.equal(p.weapon,'P');assert.equal(p.rapid,0);assert.ok(p.invincible>2);assert.equal(p.x,110);});
test('co-op continues while either player survives',()=>{const e=game({players:2});quiet(e);const p=e.state.players[0];p.lives=1;p.hp=1;p.invincible=0;e.state.bullets.push(enemyBullet(p));ticks(e,1);assert.equal(p.lives,0);assert.equal(e.state.status,'playing');});
test('game over, limited continues, and restart reset state correctly',()=>{const e=game();for(let i=0;i<4;i++){quiet(e);const p=e.state.players[0];p.lives=1;p.hp=1;p.invincible=0;e.state.bullets.push(enemyBullet(p));ticks(e,1);assert.equal(e.state.status,'gameover');assert.equal(e.continueRun(),i<3);}assert.equal(e.state.continues,0);e.start();assert.equal(e.state.continues,3);assert.equal(e.state.score,0);});
...
test('extra-life threshold awards exactly once',()=>{const e=game();quiet(e);e.state.score=14900;e.state.pickups.push({x:110,y:410,w:24,h:24,type:'R',ttl:10});ticks(e,1);assert.equal(e.state.players[0].lives,8);ticks(e,10);assert.equal(e.state.players[0].lives,8);});
