...
test('each era contains five themed enemies, varied traps, no story and three usable weapons',()=>{
  const C=require('../js/content');assert.equal(Object.keys(C.weapons).length,18);const e=createEngine();e.start();for(let i=0;i<6;i++){if(i){e.state.status='clear';e.advance();}assert.equal(stages[i].weapons.length,3);assert.equal(e.state.story,null);assert.equal(stages[i].enemies.length,5);assert.equal(new Set(stages[i].enemies.map(e=>e.kind)).size,5);assert.ok(e.state.traps.some(t=>t.kind===stages[i].trap));assert.ok(e.state.pickups.some(p=>p.kind==='weapon'&&p.weapon===stages[i].weapons[1]));assert.equal(e.state.props.filter(p=>p.kind==='lid').length,2);assert.ok(new Set(e.state.props.map(p=>p.kind)).size>=3);}
});
