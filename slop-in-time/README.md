...
| Throw equipped weapon | Hold V + Attack | Hold K + Attack | Hold RB + Attack | Throw |
| Charged special | C | J | Y | Special when ready |
| Pause | P / Escape | Shared | Start | Pause |

Each player starts with 100 stamina. Every era has one permanent +20 max stamina battery, reaching 220 if all six are collected. Eras 2, 4, and 6 each have a strength medallion granting +20% damage, reaching +60%. These upgrades apply to both co-op players, persist through deaths, stages and continues within the run, and cannot be farmed; a new run resets them. Uncollected upgrades remain reachable after a checkpoint continue. Strength improves punches, kicks, grabs, melee weapons, Specials and thrown weapons. Jabs cost 12, weapon swings 16, finishers/kicks/throws 18. Running moves at 300 instead of 205 pixels/second and drains 22 stamina/second while moving on the ground. Walking and jumping are free. Stamina recovers at 24/second whenever you are not running, including during attacks. Each attack costs more than its normal animation recovers. At low stamina, holding Attack waits only until the next hit is affordable, then attacks again automatically; there is no combat recovery lockout. An emptied sprint still needs 35 stamina before running resumes, but you can punch during that recovery. Special uses its own charge. Respawning or entering an era refills stamina.

The mascot uses a dedicated side-profile sprite sheet with four walking and four running poses, mirrored to face the travel direction. Ground enemies retain articulated painted legs and boots; airborne enemies and spectral bosses retain flight/hover animation.
...
Collect hot dogs (+15 HP), pizza (+25), hamburgers (+35), ramen (+50), retro game cartridges (+30 Special), and era-specific melee weapons. The segmented health meter shows exact current/max HP. Each era has one heart that permanently adds one maximum HP for both co-op players during the current run, surviving deaths, stages and continues. A new run starts at 100 HP; collected hearts cannot be farmed by continuing. Food cannot heal beyond maximum HP. Food and Special cartridge placements now occur once per four original supply groups (halved again from the previous update), leaving 2-3 of each per era; enemy food/cartridge drops have a combined 3% chance, down from 6%. Weapon placements and the one permanent heart per era are unchanged. Every era has three weapons, with different reach, damage, swing speed and durability; there are eighteen in total. The new weapons are street chain, coral trident, obsidian axe, storm naginata, railbreaker pick and photon hammer. Later-era melee equipment deals more damage. Hold V and Attack to throw the equipped weapon (P2: K + Attack; gamepad: RB + Attack; mobile: THROW). A throw costs 18 stamina, consumes the weapon, flies in the facing direction and deals 165% of its melee damage before strength scaling. It hits up to two enemies; a boss stops it. Throws can also be launched while airborne. Attack near a closed manhole, hatch or stone cover to lift and throw it through enemies. The open hole remains a hazard. There are only two throwable covers per era. Hit themed interactive scenery to stun foes or neutralize hazards. Use the clear lower lane to avoid overhead and tall hazards. Low hazards can be jumped. Steam rises in translucent clouds. Landing attacks and scoring knockouts charges Special. A full meter unleashes an area attack without costing health.
...
`tests/progression.test.js` covers permanent upgrades, recovery after checkpoints, all five enemy health curves, stronger weapons, thrown damage and quick tap input. `tools/cdp-drivers/slop-in-time-progression-qa.js` checks live pickup counts, shared upgrades and upgraded HUD/weapon rendering. The general browser QA also covers V + Attack and mobile THROW.
