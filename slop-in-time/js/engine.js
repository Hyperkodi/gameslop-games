(function(root){
  ...
  function createEngine({seed=1991}={}){
    ...
    const state={status:'ready',stage:0,players:[],enemies:[],props:[],pickups:[],effects:[],shots:[],events:[],inputLog:[],camera:0,tick:0,time:0,score:0,kills:0,combo:0,comboTime:0,bestCombo:0,shake:0,flash:0,hitstop:0,continues:2,gate:0,arena:false,width:3100,transition:0,difficulty:'normal',seed};
    ...
    function stageLoad(index){
      ...
      for(let i=0;i<10;i++)state.props.push({id:nextId++,x:330+i*260,y:350+(i%3)*60,hp:16,kind:i%3===0?'food':i%3===1?'weapon':'energy'});
    ...
    function continueRun(){if(state.status!=='gameover'||state.continues<=0)return;state.continues--;state.players.forEach(p=>{p.lives=rules().lives;p.energy=40;});state.status='playing';stageLoad(state.stage);}
    ...
    function spawn(kind,x,y,boss=false){
      ...
      const e={id:nextId++,kind,boss,x,y,z:0,hp,maxHp:hp,face:-1,cooldown:.65+rng()*.7,stun:0,down:0,windup:0,attack:null,vx:0,thrown:0,throwHits:[],phase:0,walk:0,dead:0};state.enemies.push(e);return e;
    }
    function beginArena(){
      state.arena=true;const center=480+state.gate*700;
      const count=(state.gate===3?2:3+state.gate)+(state.players.length-1)*2+(state.difficulty==='hard'?2:0);
      for(let i=0;i<count;i++)spawn(['grunt','swift','grunt','thrower','brute'][(i+state.stage+state.gate)%5],center+110+(i%3)*70,350+(i%4)*42);
      if(state.gate===3)spawn('boss',center+200,419,true);
      emit('arena');
    }
    ...
    function movePlayer(p,dt){
      ...
      const boundary=state.arena?480+state.gate*700+340:state.width-40;
    ...
    function enemyAttack(e,target){
      ...
      const kind=e.boss?patterns[state.stage][e.phase%3]:e.kind==='thrower'?'shot':'punch';
      e.attack={kind,x:target.x,y:target.y,face:e.face};e.windup=e.boss?.8:e.kind==='brute'?.7:.48;e.phase++;
    }
    function moveEnemy(e,dt){
      if(e.hp<=0&&e.thrown<=0){e.dead=Math.max(0,e.dead-dt);return;}
      e.stun=Math.max(0,e.stun-dt);e.down=Math.max(0,e.down-dt);e.cooldown=Math.max(0,e.cooldown-dt);e.walk+=dt*8;
      ...
      e.x=clamp(e.x+e.vx*dt,state.camera+20,Math.min(state.width-30,480+state.gate*700+350));e.vx*=Math.exp(-6*dt);
      ...
      if(e.windup>0){e.windup-=dt;if(e.windup<=0){const a=e.attack;e.attack=null;e.cooldown=e.boss?1.1:e.kind==='swift'?.8:1.3;
        if(['shot','volley','pulse'].includes(a.kind)){
        ...
        else if(a.kind==='blink'){fx('burst',e.x,e.y-40);e.x=clamp(a.x-a.face*100,state.camera+30,Math.min(state.width-40,480+state.gate*700+320));e.y=clamp(a.y,335,487);e.face=a.face;e.cooldown=.22;fx('burst',e.x,e.y-40);}
        else if(a.kind==='pounce'){e.x=clamp(a.x,state.camera+30,Math.min(state.width-40,480+state.gate*700+320));e.y=clamp(a.y,335,487);fx('slam',e.x,e.y,{life:.5,max:.5});for(const p of alive)if(Math.abs(p.x-e.x)<85&&Math.abs(p.y-e.y)<55&&p.z<35)hurtPlayer(p,20,Math.sign(p.x-e.x)||1);emit('slam');}
      ...
      const speed=e.kind==='swift'?115:e.kind==='brute'?57:e.boss?78:83;
    ...
    function tick(dt=STEP){
      ...
      state.players.forEach(p=>movePlayer(p,dt));state.enemies.forEach(e=>moveEnemy(e,dt));
      ...
      if(!state.arena&&state.gate<4&&lead>480+state.gate*700-330)beginArena();
      if(state.arena&&!state.enemies.some(e=>e.hp>0)){
        state.arena=false;state.gate++;state.players.forEach(p=>p.energy=clamp(p.energy+12,0,100));emit('gate');
        if(state.gate===4){state.status='clear';state.shots=[];release();emit('clear');}
      }
  ...
  const api={createEngine,stages,difficulties,STEP};root.SlopInTime=Object.assign(root.SlopInTime||{},api);if(typeof module!=='undefined')module.exports=api;
